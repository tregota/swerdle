...
const TileRow = ({ letters = '', types = '', tileSize, wordLength  = 5, gameOver }) => {
  ...
  return (
    <TileRowContainer>
      {Array.from(Array(wordLength).keys()).map((index) => <TileComponent key={index} flipped={!!types} position={index} size={tileSize} letter={tileLetters[index]} type={typeMapping[tileTypes[index]]} gameOver={gameOver} />)}
    </TileRowContainer>
  )
}
